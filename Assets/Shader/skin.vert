...
void main()
{
    ...
    // remember to initialize the local variables.
	...
	v2fPos = (MATRIX_WORLD * finalXForm * vec4(position, 1.0)).xyz;
	
	gl_Position = MATRIX_PERS_VIEW * vec4(v2fPos, 1.0);
	
	// normal in world space
	// since we only allow uniform scale on animation,
	// no need to calculate normal matrix by inverse
	mat3 finalXForm_normal = mat3(finalXForm);	
	v2fNormal = MATRIX_NORMAL_W * finalXForm_normal * normal;
